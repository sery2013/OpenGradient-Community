...
async function mintCardNFT() {
    ...
    try {
        // 1. Создаём провайдер для взаимодействия с сетью
        const provider = new ethers.JsonRpcProvider("https://rpc.ritualfoundation.org");

        // 2. Получаем информацию о сети (nonce, gasPrice)
        const nonce = await provider.getTransactionCount(globalWallet.address, "pending");

        // Получаем gasPrice напрямую через RPC, т.к. сеть не поддерживает EIP-1559
        let gasPrice;
        try {
            ...
            console.warn("Could not fetch gasPrice via RPC, using fallback");
...
// === NFT CARD: ГЕНЕРАЦИЯ CANVAS (600x800 - VERTICAL) ===
async function generateCardCanvas(username, stats) {
    ...
    metrics.forEach((m, i) => {
        ...
        ctx.fillRect(30, yPos - 10, cellW, cellH);
        // Рамка ячейки
        ctx.strokeStyle = 'rgba(111, 227, 209, 0.2)';
        ctx.lineWidth = 1;
        ctx.strokeRect(30, yPos - 10, cellW, cellH);
        // Иконка и название
        ctx.fillStyle = '#a9ddd3';
        ctx.font = '20px Segoe UI, sans-serif';
        ctx.textAlign = 'left';
        ctx.fillText(`${m.icon} ${m.label}`, 50, yPos + 15);
        // Значение
        ctx.fillStyle = '#6fe3d1';
        ctx.font = 'bold 24px Segoe UI, sans-serif';
        ctx.fillText(Number(m.val).toLocaleString(), W - 100, yPos + 15);
        yPos += cellH + 10;
    });

    // 7. Разделительная линия перед футером
    ctx.strokeStyle = 'rgba(111, 227, 209, 0.3)';
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(30, yPos - 10);
    ctx.lineTo(W - 30, yPos - 10);
    ctx.stroke();
...
function renderTable() {
    ...
    pageData.forEach(stats => {
        ...
        tr.insertAdjacentHTML('beforeend', `<td></td>`); // Card column
