...
// Убрали let currentCardData = ...; — теперь используем window.currentCardData
...
function showCardModal(username) {
    ...
    if (!user) {
        console.error("❌ User not found in ", username);
        return;
    }
...
async function generateCardCanvas(username, stats) {
    ...
    // ❌ Убрали window.currentCardData.imageData = ...; — не нужно
